...
import com.example.firebasenotepad.databinding.EditgameFragmentBinding
...
class EditGameFragment:Fragment() {
    override fun onCreateView(
        ...
        val binding = EditgameFragmentBinding.inflate(inflater)
        binding.lifecycleOwner = this

        val gameIndex = EditGameFragmentArgs.fromBundle(
            requireArguments()
        ).gameIndex

        val factory = EditGameViewModel.Factory(gameIndex)
        val viewModel = ViewModelProvider(this,factory).get(EditGameViewModel::class.java)
